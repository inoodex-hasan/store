...
<div class="row justify-content-center">
    <div class="col">
        <div class="card p-4 shadow">
            <h2 class=" mb-3">Update daily Total sale</h2>
            <p class="mb-3">Update your total sale Manually</p>
            <form method="POST" action="{{ route('dailySales.update',$dailySale->id) }}">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="date" class="form-label fw-bold">Date<span class="text-danger">*</span></label>
                    <input type="date" class="form-control" name="date" required value="{{$dailySale->date}}">
                </div>

                <div class="mb-3">
                    <label for="description" class="form-label fw-bold">Description</label>
                    <textarea class="form-control" name="description" rows="3">{{$dailySale->description}}</textarea>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        ...
                        <input onchange="calculateTotal()" type="number" class="form-control" name="card_amount" id="card_amount" value="{{$dailySale->card_amount}}">
                    </div>
                    <div class="col-md-4">
                        <label for="cashAmount" class="form-label">Cash amount</label>
                        <input onchange="calculateTotal()" type="number" class="form-control" name="cash_amount" id="cash_amount" value="{{$dailySale->cash_amount}}">
                    </div>
                    <div class="col-md-4">
                        <label for="othersAmount" class="form-label">Others amount</label>
                        <input onchange="calculateTotal()" type="number" class="form-control" name="others_amount" id="others_amount" value="{{$dailySale->other_amount}}">
                    </div>
                </div>

                <div class="mb-3">
                    <label for="totalAmount" class="form-label fw-bold">Total amount</label>
                    <input type="number" class="form-control" name="total_amount" id="total_amount" value="{{$dailySale->card_amount + $dailySale->cash_amount + $dailySale->other_amount}}" readonly>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select js-example-basic-single" name="assigned_person_id[]" required multiple>
                        ...
                        @foreach ($users as $user)
                            <option value="{{$user->id}}" {{in_array($user->id, explode(',', $dailySale->assigned_person_id)) ? 'selected' : ''}}>{{$user->name}}</option>
                        @endforeach
                    </select>
                </div>
